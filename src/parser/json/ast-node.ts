...
export class ASTNode<TValue = unknown> {
    ...
    contains(offset: number, includeRightBound = false): boolean {
        return (
    ...
    validate(
        ...
        if (Array.isArray(schema.anyOf)) {
            testAlternatives(schema.anyOf as JSONSchema[], false)
        }
        if (Array.isArray(schema.oneOf)) {
            testAlternatives(schema.oneOf as JSONSchema[], true)
        }
